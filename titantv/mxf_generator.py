...
def generate_mxf(data, output_path):
    ...
    if not all(key in data for key in ["lineup", "channels", "schedule"]):
        logging.error("Missing required data for MXF generation")
        return
    ...
    try:
        ...
        logging.error(f"Error writing channel data: {e}")
        return
    ...
    try:
        ...
        logging.error(f"Error writing program data: {e}")
        return
    ...
    try:
        ...
        logging.error(f"Error writing schedule entries: {e}")
        return
